...
export function useGameEngine(canvasRef, authToken, onGameOver, apiBase) {
  ...
  const drawPlayer = (ctx, player, canvas, isRainbow) => {
    ...
    const gradient = ctx.createLinearGradient(player.x, y, player.x + player.width, y + player.height);
    ...
    ctx.fillText('XANO', player.x + player.width / 2, y + player.height / 2 + 4);
    if (isRainbow) {
      for (let i = 0; i < 10; i++) {
        ctx.fillStyle = `hsl(${(Date.now() * 0.1 + i * 36) % 360}, 100%, 50%)`;
        ctx.globalAlpha = 0.3 - i * 0.03;
        ctx.fillRect(player.x - i * 5, y, player.width, player.height);
      }
    }
    ctx.restore();
  };
